...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 20 00:40:07 2026
...
def get_image_url(card):
    """
    Prefer border_crop (JPEG, no border, good for display).
    Falls back to art_crop then png if border_crop isn't available.
    """
...
async def download_token(session, semaphore, card):
    ...
    async with semaphore:
        try:
            ...
            # Scryfall border_crop is a JPEG — open it, convert to RGB,
            # resize to display resolution, then save as PNG for the ESP32.
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [download_token(session, semaphore, card) for card in tokens]
